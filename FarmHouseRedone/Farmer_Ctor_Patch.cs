...
{
    internal class Farmer_Ctor_Patch
    {
        public static void Postfix(Farmer __instance)
        {
            ...
            if (__instance.currentLocation != null && __instance.currentLocation is FarmHouse)
            {
                ...
                __instance.mostRecentBed =
                    StardewValley.Utility.PointToVector2(
                        FarmHouseStates.getBedSpot(__instance.currentLocation as FarmHouse)) * 64f;
            }
            else
            {
                Logger.Log("Current location was not a farmhouse; getting bed spot from " + __instance.Name +
                           "'s farmhouse...");
                foreach (var location in Game1.locations)
                    if (location is FarmHouse && (location as FarmHouse).owner == __instance)
                    {
                        ...
                        __instance.mostRecentBed =
                            StardewValley.Utility.PointToVector2(FarmHouseStates.getBedSpot(location as FarmHouse)) *
                            64f;
                    }
            }
