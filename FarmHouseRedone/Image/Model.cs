...
namespace FarmHouseRedone.Image
{
    public class Model
    {
        ...
        {
            var distanceToVolumePoint =
                Math.Sqrt(Math.Pow(rayPosition.X - position.X, 2) + Math.Pow(rayPosition.Y - position.Y, 2));
            return distanceToVolumePoint <= radius;
        }

        public int getLuminenceDiffuse(Vector2 renderRay, Vector3 sunRay)
        {
            var positionOnSphere = getSurfacePoint(renderRay);

            //return (int)((-positionOnSphere.Y / (radius * 2)) * 255) + 128;

            //return (int)((positionOnSphere.Z / (radius * 2)) * 255) + 128;

            var angleY = Math.Atan(positionOnSphere.Z / -positionOnSphere.Y);
        ...
        public Vector3 getSurfacePoint(Vector2 renderRay)
        {
            ...
            var minorRadius = Math.Sqrt(Math.Pow(radius, 2) - Math.Pow(renderRay.X - position.X, 2));
            var z = Math.Sqrt(Math.Pow(minorRadius, 2) - Math.Pow(renderRay.Y - position.Y, 2));
