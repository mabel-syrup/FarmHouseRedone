...
namespace FarmHouseRedone.MapUtilities
{
    public static class MapMerger
    {
        ...
        public static void pasteMap(GameLocation location, Map map, int pasteX, int pasteY,
            int pasteMode = PASTE_NONDESTRUCTIVE)
        {
            var equivalentSheets = SheetHelper.getEquivalentSheets(location, map);
            var mapSize = getMapSize(map);
            var hostMapSize = getMapSize(location.map);
            for (var x = 0; x < mapSize.X && pasteX + x < hostMapSize.X; x++)
            for (var y = 0; y < mapSize.Y && pasteY + y < hostMapSize.Y; y++)
            ...
            foreach (var pair in map.Properties)
            {
                ...
                {
                    Logger.Log("Host map did not have a '" + propertyName + "' property, setting it to '" +
                               propertyName + "'...");
                    location.map.Properties.Add(propertyName, value);
                }
                else
                {
                    var houseValue = location.map.Properties[propertyName];
        ...
        public static void pasteTile(Map map, Map sectionMap, int x, int y, int mapX, int mapY,
            Dictionary<TileSheet, TileSheet> equivalentSheets, int pasteMode = PASTE_NONDESTRUCTIVE)
        {
            pasteTileInLayer(map, sectionMap, x, y, mapX, mapY, "Back", equivalentSheets, pasteMode == 0);
            pasteTileInLayer(map, sectionMap, x, y, mapX, mapY, "Buildings", equivalentSheets,
                pasteMode == 0 || pasteMode == 2);
            pasteTileInLayer(map, sectionMap, x, y, mapX, mapY, "Front", equivalentSheets,
                pasteMode == 0 || pasteMode == 2);
            pasteTileInLayer(map, sectionMap, x, y, mapX, mapY, "AlwaysFront", equivalentSheets,
                pasteMode == 0 || pasteMode == 2);
        }

        public static void pasteTileInLayer(Map map, Map sectionMap, int x, int y, int mapX, int mapY, string layer,
            Dictionary<TileSheet, TileSheet> equivalentSheets, bool destructive)
        {
            if (sectionMap.GetLayer(layer) == null)
                return;
            if (sectionMap.GetLayer(layer).Tiles[x, y] != null)
            {
                var sectionTile = sectionMap.GetLayer(layer).Tiles[x, y];
                if (sectionTile is AnimatedTile)
                {
                    var framesCount = (sectionTile as AnimatedTile).TileFrames.Length;
                    var frames = new StaticTile[framesCount];
                    for (var i = 0; i < framesCount; i++)
                    {
                        var frame = (sectionTile as AnimatedTile).TileFrames[i];
                        frames[i] = new StaticTile(map.GetLayer(layer), equivalentSheets[sectionTile.TileSheet],
                            frame.BlendMode, frame.TileIndex);
                    }

                    map.GetLayer(layer).Tiles[mapX, mapY] = new AnimatedTile(map.GetLayer(layer), frames,
                        (sectionTile as AnimatedTile).FrameInterval);
                }
                else
                {
                    map.GetLayer(layer).Tiles[mapX, mapY] = new StaticTile(map.GetLayer(layer),
                        equivalentSheets[sectionTile.TileSheet], sectionTile.BlendMode, sectionTile.TileIndex);
                }

                if (sectionTile.Properties.Keys.Count > 0)
                    foreach (var pair in sectionTile.Properties)
        ...
        public static void setMapTileIndexIfOnTileSheet(Map map, int x, int y, int index, string layer,
            int tileSheetToMatch, Rectangle region)
        {
        ...
        {
            setMapTileIndexIfOnTileSheet(map, x, y, getWallIndex(map, x, y, "Back", index), "Back",
                SheetHelper.getTileSheet(map, "walls_and_floors"), new Rectangle(0, 0, 16, 21));
            setMapTileIndexIfOnTileSheet(map, x, y, getWallIndex(map, x, y, "Buildings", index), "Buildings",
                SheetHelper.getTileSheet(map, "walls_and_floors"), new Rectangle(0, 0, 16, 21));
            setMapTileIndexIfOnTileSheet(map, x, y, getWallIndex(map, x, y, "Front", index), "Front",
                SheetHelper.getTileSheet(map, "walls_and_floors"), new Rectangle(0, 0, 16, 21));
        }

        public static void DoSetVisibleWallpaper(int whichRoom, int which, DecoratableLocation location)
        {
            var client = OtherLocations.FakeDecor.FakeDecorHandler.getClient(location);
            if (client != null)
            {
                client.updateMap();
            ...
            var index = which % 16 + which / 16 * 48;
            ...
            {
                Logger.Log(
                    "Wall rectangle exceeded walls count!  You can ignore this if the farmhouse just upgraded, or you installed a new farmhouse mod.",
                    LogLevel.Warn);
                return;
            }

            //Find the region to paste in
            var rectangle = walls[whichRoom];
        ...
        internal static int getFloorIndex(Map map, int x, int y, string layer, int destinationIndex)
        {
            ...
            var horizontalOffset = currentIndex % 2;
            var verticalOffset = (currentIndex - 336) / 16 % 2;
        ...
        {
            setMapTileIndexIfOnTileSheet(map, x, y, getFloorIndex(map, x, y, "Back", index), "Back",
                SheetHelper.getTileSheet(map, "walls_and_floors"), new Rectangle(0, 21, 16, 10));
            setMapTileIndexIfOnTileSheet(map, x, y, getFloorIndex(map, x, y, "Buildings", index), "Buildings",
                SheetHelper.getTileSheet(map, "walls_and_floors"), new Rectangle(0, 21, 16, 10));
            setMapTileIndexIfOnTileSheet(map, x, y, getFloorIndex(map, x, y, "Front", index), "Front",
                SheetHelper.getTileSheet(map, "walls_and_floors"), new Rectangle(0, 21, 16, 10));
        }
        ...
        internal static void setMapTileIndexesInSquare(Map map, Rectangle floor, int x, int y, int index)
        {
            if (floor.Contains(x, y))
                setMapTileIndexIfOnTileSheet(map, x, y, index, "Back",
                    SheetHelper.getTileSheet(map, "walls_and_floors"), new Rectangle(0, 21, 16, 10));
            if (floor.Contains(x + 1, y))
                setMapTileIndexIfOnTileSheet(map, x + 1, y, index + 1, "Back",
                    SheetHelper.getTileSheet(map, "walls_and_floors"), new Rectangle(0, 21, 16, 10));
            if (floor.Contains(x, y + 1))
                setMapTileIndexIfOnTileSheet(map, x, y + 1, index + 16, "Back",
                    SheetHelper.getTileSheet(map, "walls_and_floors"), new Rectangle(0, 21, 16, 10));
            if (floor.Contains(x + 1, y + 1))
                setMapTileIndexIfOnTileSheet(map, x + 1, y + 1, index + 17, "Back",
                    SheetHelper.getTileSheet(map, "walls_and_floors"), new Rectangle(0, 21, 16, 10));
        }
