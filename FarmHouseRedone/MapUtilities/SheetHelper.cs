...
namespace FarmHouseRedone.MapUtilities
{
    public static class SheetHelper
    {
        ...
        {
            var equivalentSheets = new Dictionary<TileSheet, TileSheet>();
            foreach (var sheet in subMap.TileSheets)
            {
                foreach (var locSheet in location.map.TileSheets)
                    ...
                    {
                        Logger.Log(cleanImageSource(locSheet.ImageSource) + " was not the same as " +
                                   cleanImageSource(sheet.ImageSource));
                    }
                ...
                {
                    Logger.Log(
                        "No equivalent sheet found for " + sheet.ImageSource + "!  Using default (" +
                        location.map.TileSheets[0].ImageSource + ")", StardewModdingAPI.LogLevel.Warn);
        ...
        {
            for (var sheetIndex = 0; sheetIndex < map.TileSheets.Count; sheetIndex++)
            {
                var sheet = map.TileSheets[sheetIndex];
                if (sheet.ImageSource.Contains(sourceToMatch))
                    return sheetIndex;
            }

            Logger.Log("Couldn't find any tilesheet that matches the source \"" + sourceToMatch + "\"!  Using index 0.",
        ...
        {
            return map.GetLayer(layer).Tiles[x, y] != null && map.GetLayer(layer).Tiles[x, y].TileSheet
                .Equals((object) map.TileSheets[tileSheetToMatch]);
        }

        public static bool isTileOnSheet(Map map, string layer, int x, int y, int tileSheetToMatch, Rectangle region)
        {
            return isTileOnSheet(map, layer, x, y, tileSheetToMatch) &&
                   isIndexInRect(map.GetLayer(layer).Tiles[x, y].TileIndex, map.TileSheets[tileSheetToMatch], region);
        }

        public static bool isIndexInRect(int index, TileSheet sheet, Rectangle region)
        {
            var x = index % sheet.SheetSize.Width;
            var y = index / sheet.SheetSize.Width;
